...
### Memory `#memory`
{: #memory }
사용자에게 지속적으로 적용되는 업무 맥락. 회사 규정은 여기 넣지 않는다. 그건 [Knowledge](#knowledge)다. 「이 사용자는 결과를 표가 아니라 유형별 목록으로 받는다」가 여기 들어간다. 올린 파일이 그대로 남는 Knowledge와 달리, 이름과 이유를 붙여 다시 쓴 것이 남는다. 28일 동안 쓰지 않으면 지워진다.
