...
{: #standard-harness }
규칙 기반 Agent와 정해진 대화를 맡는 쪽. 내가 그린 [토픽](#topic)과 규칙을 그대로 따라 예측 가능하게 움직인다. Copilot Studio 초급 과정이 다루는 하네스다.
...
### 토픽 (topic) `#topic`
{: #topic }
사용자의 말을 미리 정해 둔 경로로 분기시키는 [표준 하네스](#standard-harness)의 부품. **GHCP 하네스에는 없다.** 초급 과정의 개념을 그대로 옮겨 오면 가장 먼저 어긋난다.
...
### Sandbox `#sandbox`
{: #sandbox }
파일과 데이터를 실제로 다루는 실행 공간. 파일 자체를 열어 전부 읽고 계산한다. 요약본을 건네받는 것과 다르다. 오늘 1,000행 대사가 여기서 수행된다.
...
### Memory `#memory`
{: #memory }
사용자에게 지속적으로 적용되는 업무 맥락. 회사 규정은 여기 넣지 않는다. 그건 [Knowledge](#knowledge)다. 「이 사용자는 결과를 표가 아니라 사람별 목록으로 받는다」가 여기 들어간다. 올린 파일이 그대로 남는 Knowledge와 달리, 이름과 이유를 붙여 다시 쓴 것이 남는다. 28일 동안 쓰지 않으면 지워진다.

### Workflow `#workflow`
{: #workflow }
언제 시작하고 어떤 순서로 진행하며 결과에 따라 어디로 갈지를 통제하는 구조. 판단은 [Agent](#agent)에게 맡기고, 프로세스는 여기서 통제한다.

### Copilot Credits `#copilot-credits`
{: #copilot-credits }
GHCP 하네스의 사용량 기반 과금 단위. 만드는 것·테스트하는 것·쓰는 것이 모두 소비한다. 전체 데이터를 읽어 처리하면 그만큼 더 소비한다.
...
### 증빙 면제 `#exemption`
{: #exemption }
증빙 대장에 올리지 않아도 되는 건. 규정 §3.3이 정한 정기결제 계약 건이 여기 해당한다. **명세에는 있고 대장에는 없지만 불일치가 아니다.** 규정을 읽지 않으면 증빙 누락으로 잘못 분류된다.
