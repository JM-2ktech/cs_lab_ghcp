...
3. 제안된 초안을 확인합니다. 제목과 장문 문항 하나가 나옵니다.

    ![경리팀 요청사항 제목과 요청사항 문항 하나가 제안된 화면](../assets/lab4/lab4-03.png)

4. 세부 정보를 더 넣어 다시 제안받고, **Copilot으로 계속하기** 를 선택합니다. 문항이 **둘**이 됩니다.

    ![요청사항과 연락처 두 문항으로 늘어난 초안](../assets/lab4/lab4-04.png)

    {: .important }
    **요청사항 문항에 선택지가 없습니다.** 「비용 문의 / 정산 요청 / 기타」로 고르게 만들면 오늘 배울 것이 사라집니다. 실제 요청자는 갈래를 골라 주지 않습니다.

<!-- ☐ 미확인 — 4번에서 무엇을 더 입력해 문항이 둘로 늘었는지 확정되지 않았다.
     화면에 「Copilot에 세부 정보를 더 제공하여 초안을 미세 조정」 입력창이 있고,
     3.png(문항 1개) 와 4.png(문항 2개) 사이에 그 입력이 있었다.
     그 문장을 받아 적어 본문의 복붙 블록으로 올린다. -->

5. **응답 수집** 에서 양식 링크를 복사해 둡니다. C단 끝에서 씁니다.

    ![촬영: 응답 수집에서 링크를 복사하는 화면](../assets/lab4/lab4-05.png)

### B. 워크플로를 만듭니다

{: start="6" }
6. Copilot Studio 왼쪽 메뉴 **워크플로** 에서 **새 워크플로** 를 선택합니다.

    ![워크플로 목록과 새 워크플로 단추](../assets/lab4/lab4-06.png)

    {: .note }
    단추 오른쪽 화살표에서 종류를 고릅니다. 같은 「워크플로」라도 과금이 구분됩니다. 오늘 쓰는 쪽은 크레딧이고, 클래식은 라이선스입니다.

7. 캔버스의 **시작** 노드를 선택하고, 트리거 유형에서 **커넥터** 를 고릅니다.

    ![시작 노드와 트리거 유형 목록. 커넥터 항목](../assets/lab4/lab4-07.png)

    {: .note }
    왼쪽 팔레트에 오늘 쓸 것이 다 있습니다. **에이전트 · 분류 · Copilot · 인적 검토 · 커넥터** 입니다.

8. Forms 커넥터를 고르고, 양식 목록에서 A단에서 만든 양식을 선택합니다.

    ![양식 목록에서 경리팀 요청사항을 고르는 화면](../assets/lab4/lab4-08.png)

    {: .note }
    연결은 로그인한 계정으로 자동 생성됩니다. 동의 화면이 뜨지 않습니다.

9. 워크플로 이름을 아래로 바꿉니다. 괄호 안은 본인 이름입니다.

    ```
    경리팀 요청 접수 (홍길동)
    ```

    ![이름을 바꾼 워크플로와 새 응답이 제출되는 경우 트리거 노드](../assets/lab4/lab4-09.png)

10. 응답 본문을 가져오는 동작을 추가합니다. 뒤 단계가 이 값을 씁니다.

     ![촬영: 응답 본문 가져오기](../assets/lab4/lab4-10.png)

11. **저장** 합니다. 배너를 읽습니다. **게시해야 실행할 수 있습니다.**

     ![촬영: 저장 안내 배너](../assets/lab4/lab4-11.png)

     {: .important }
     저장과 게시가 다릅니다. 저장은 만들던 것을 남기는 조작이고, **게시해야 트리거가 동작하고 실행됩니다.** 게시 전에는 실행 버튼도 눌리지 않습니다.

12. **게시** 를 선택합니다. 상태 뱃지가 `초안` 에서 `게시됨` 으로 바뀝니다.

     ![촬영: 게시 전후 뱃지](../assets/lab4/lab4-12.png)
...
13. 양식에 아무 글이나 하나 넣어 봅니다. 실행 기록이 남는지 확인합니다.
     ...
     ![촬영: 실행 기록](../assets/lab4/lab4-13.png)
...
14. 다음 노드로 **분류**를 추가합니다.
     ...
     ![촬영: 분류 노드 추가](../assets/lab4/lab4-14.png)

15. 연결을 만들라고 하면 만듭니다. 동의 화면이 뜨면 승인합니다.

16. 분류할 대상으로 10번의 **응답 본문**을 지정합니다.

     ![촬영: 분류 입력 지정](../assets/lab4/lab4-16.png)

17. 갈래를 **넷**으로 늘립니다. 기본은 셋입니다.

     ![촬영: 갈래 추가](../assets/lab4/lab4-17.png)
...
18. 갈래마다 이름과 설명을 적습니다.
     ...
     ![촬영: 갈래 정의](../assets/lab4/lab4-18.png)
...
21. 실행 기록에서 어느 갈래로 갔는지 봅니다. `규정 문의` 입니다.
     ...
     ![촬영: 분류 결과](../assets/lab4/lab4-21.png)
...
25. `대사 요청` 갈래에 **에이전트** 노드를 추가합니다.
     ...
     ![촬영: 에이전트 노드 추가](../assets/lab4/lab4-25.png)

26. 목록에서 **본인 이름이 붙은** `비용 처리 담당` 을 고릅니다. 같은 환경에 여러 사람의 것이 함께 보입니다.

     ![촬영: 에이전트 선택](../assets/lab4/lab4-26.png)
...
27. 에이전트에게 넘길 것을 지정합니다. 응답 본문과 요청자를 함께 넘깁니다.
     ...
     ![촬영: 입력 지정](../assets/lab4/lab4-27.png)

28. 저장하고 다시 게시한 뒤, 22번의 문장을 다시 넣습니다.

29. 실행 기록을 열어 에이전트의 답이 돌아왔는지 봅니다.

     ![촬영: 에이전트 응답](../assets/lab4/lab4-29.png)

30. 돌아온 내용을 읽습니다. Lab 1 에서 미리 보기로 받던 것과 같습니다. **사람이 창을 열지 않았는데도 같은 결과가 나옵니다.**
...
31. `규정 문의` 갈래에 **M365 Copilot** 노드를 추가합니다.
     ...
     ![촬영: M365 Copilot 노드](../assets/lab4/lab4-31.png)

32. 연결을 만들라고 하면 만듭니다.

33. 물어볼 내용으로 응답 본문을 넘깁니다.

     ![촬영: 질문 지정](../assets/lab4/lab4-33.png)

34. 저장하고 다시 게시한 뒤, 20번의 문장을 다시 넣습니다.

35. 답이 돌아오는지 봅니다.

     ![촬영: M365 Copilot 응답](../assets/lab4/lab4-35.png)
...
41. `해당 없음` 갈래에 **Teams 메시지 보내기**를 추가합니다.
     ...
     ![촬영: 해당 없음 갈래의 Teams 노드](../assets/lab4/lab4-41.png)
...
45. Teams에 메시지가 오는지 확인합니다.
     ...
     ![촬영: 관계없는 요청이 Teams로 온 화면](../assets/lab4/lab4-45.png)
