...
{: start="16" }
16. 다음 노드로 **분류**를 추가합니다.

     ![촬영: 분류 노드 추가](../assets/lab4/lab4-16.png)

17. 연결을 만들라고 하면 만듭니다. 동의 화면이 뜨면 승인합니다.

18. 분류할 대상으로 12번 노드가 가져온 **요청사항** 값을 지정합니다.

     ![촬영: 분류 입력 지정](../assets/lab4/lab4-18.png)

19. 갈래를 **하나 더합니다.** 직접 정할 갈래가 셋이 됩니다.

     ![촬영: 갈래 추가](../assets/lab4/lab4-19.png)

     {: .important }
     **기본으로 오는 갈래는 둘이고, 거기에 「기타」가 붙어 셋으로 보입니다.** 기타는 지우지 않습니다. 어느 갈래에도 걸리지 않은 것이 그리로 가므로, 비용 처리와 관계없는 요청을 받을 갈래를 따로 만들지 않습니다.

     {: .warning }
     **아래에 넣을 한글은 메모장에 미리 적어 두고 붙여넣습니다.** 이 입력칸에서는 한글이 제대로 입력되지 않습니다. 20번과 21번의 표를 먼저 메모장에 옮겨 두고 그때그때 복사해 씁니다.

20. 갈래마다 이름과 설명을 적습니다.
     ...
     ![촬영: 갈래 정의](../assets/lab4/lab4-20.png)

21. 갈래마다 **예제**를 둘씩 넣습니다. 설명만으로 갈리지 않을 때 이 문장들이 기준이 됩니다.

     | 갈래 | 예제 |
     |---|---|
     | `규정 문의` | 출장 숙박비 한도가 얼마인가요?<br>법인카드로 산 도서도 증빙을 올려야 하나요? |
     | `대사 요청` | 6월 카드 명세랑 증빙 대장 좀 맞춰 봐 주세요<br>증빙 누락 건이 있는지 확인 부탁드립니다 |
     | `사람 판단` | 이 건이 접대비인지 회식비인지 판단이 안 됩니다<br>승인권자가 팀장인지 본부장인지 확인이 필요합니다 |

     ![촬영: 갈래별 예제 입력](../assets/lab4/lab4-21.png)
...
{: start="28" }
28. `대사 요청` 갈래에 **에이전트** 노드를 추가합니다.

     ![촬영: 에이전트 노드 추가](../assets/lab4/lab4-28.png)

29. 목록에서 **본인 이름이 붙은** `비용 처리 담당` 을 고릅니다. 같은 환경에 여러 사람의 것이 함께 보입니다.

     ![촬영: 에이전트 선택](../assets/lab4/lab4-29.png)

     {: .note }
     목록에 없으면 준비의 게시가 끝나지 않은 것입니다. **게시된 에이전트만 여기 표시됩니다.**

30. 에이전트에게 넘길 것을 지정합니다. 응답 본문과 요청자를 함께 넘깁니다.

     ![촬영: 입력 지정](../assets/lab4/lab4-30.png)

31. 저장하고 다시 게시한 뒤, 25번의 문장을 다시 넣습니다.

32. 실행 기록을 열어 에이전트의 답이 돌아왔는지 봅니다.

     ![촬영: 에이전트 응답](../assets/lab4/lab4-32.png)

33. 돌아온 내용을 읽습니다. Lab 1 에서 미리 보기로 받던 것과 같습니다. **사람이 창을 열지 않았는데도 같은 결과가 나옵니다.**

### E. M365 Copilot 노드로 규정 문의를 받습니다

{: start="34" }
34. `규정 문의` 갈래에 **M365 Copilot** 노드를 추가합니다.

     ![촬영: M365 Copilot 노드](../assets/lab4/lab4-34.png)

35. 연결을 만들라고 하면 만듭니다.

36. 물어볼 내용으로 응답 본문을 넘깁니다.

     ![촬영: 질문 지정](../assets/lab4/lab4-36.png)

37. 저장하고 다시 게시한 뒤, 23번의 문장을 다시 넣습니다.

38. 답이 돌아오는지 봅니다.

     ![촬영: M365 Copilot 응답](../assets/lab4/lab4-38.png)
...
{: start="39" }
39. `대사 요청` 갈래의 에이전트 노드 뒤에 **Teams 메시지 보내기**를 붙입니다.

     ![촬영: Teams 노드](../assets/lab4/lab4-39.png)

40. 받는 사람을 요청자로 지정하고 본문에 에이전트의 답을 넣습니다.

     ![촬영: 메시지 구성](../assets/lab4/lab4-40.png)

41. `규정 문의` 갈래에도 같은 것을 붙입니다.

42. `사람 판단` 갈래에는 **담당자에게** 보냅니다. 여기는 자동으로 처리하지 않습니다.

     ![촬영: 사람 판단 갈래](../assets/lab4/lab4-42.png)

43. 양식에 25번의 문장을 넣고 Teams에 실제로 도착하는지 봅니다.

     ![촬영: Teams 도착](../assets/lab4/lab4-43.png)
...
{: start="44" }
44. **기타** 갈래에 **Teams 메시지 보내기**를 추가합니다.

     ![촬영: 기타 갈래의 Teams 노드](../assets/lab4/lab4-44.png)

45. 본문에 아래를 그대로 입력합니다.
...
46. 요청 본문을 함께 실어 보냅니다. 사람이 읽고 판단할 수 있어야 합니다.

47. 양식에 **비용과 관계없는 글**을 하나 넣습니다.

    ```
    사무실 형광등이 하나 나갔는데 어디에 말하면 되나요?
    ```

48. Teams에 메시지가 오는지 확인합니다.

     ![촬영: 관계없는 요청이 Teams로 온 화면](../assets/lab4/lab4-48.png)
