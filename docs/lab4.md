...
<!-- ☐ 미확정(2026-09-08) — 이 스텝을 「확인합니다」로 둘지 「모든 사람이 응답할 수 있음으로 바꿉니다」로
     ...
     트리거가 돌지 않는 원인을 좁힐 때 이 자리를 함께 본다. -->
...
12. 응답 본문을 가져오는 동작을 추가합니다. 뒤 단계가 이 값을 씁니다.

     ![촬영: 응답 본문 가져오기](../assets/lab4/lab4-12.png)

13. **저장** 합니다. 배너를 읽습니다. **게시해야 실행할 수 있습니다.**

     ![촬영: 저장 안내 배너](../assets/lab4/lab4-13.png)

     {: .important }
     저장과 게시가 다릅니다. 저장은 만들던 것을 남기는 조작이고, **게시해야 트리거가 동작하고 실행됩니다.** 게시 전에는 실행 버튼도 눌리지 않습니다.

14. **게시** 를 선택합니다. 상태 뱃지가 `초안` 에서 `게시됨` 으로 바뀝니다.

     ![촬영: 게시 전후 뱃지](../assets/lab4/lab4-14.png)

     {: .note }
     준비에서 게시한 것은 **에이전트** 입니다. 이것은 **워크플로** 게시입니다. 둘은 별개입니다.

15. 양식에 아무 글이나 하나 넣어 봅니다. 실행 기록이 남는지 확인합니다.

     ![촬영: 실행 기록](../assets/lab4/lab4-15.png)
...
18. 분류할 대상으로 12번의 **응답 본문**을 지정합니다.

     ![촬영: 분류 입력 지정](../assets/lab4/lab4-18.png)

19. 갈래를 **넷**으로 늘립니다. 기본은 셋입니다.
...
{: start="27" }
27. `대사 요청` 갈래에 **에이전트** 노드를 추가합니다.

     ![촬영: 에이전트 노드 추가](../assets/lab4/lab4-27.png)

28. 목록에서 **본인 이름이 붙은** `비용 처리 담당` 을 고릅니다. 같은 환경에 여러 사람의 것이 함께 보입니다.

     ![촬영: 에이전트 선택](../assets/lab4/lab4-28.png)

     {: .note }
     목록에 없으면 준비의 게시가 끝나지 않은 것입니다. **게시된 에이전트만 여기 표시됩니다.**

29. 에이전트에게 넘길 것을 지정합니다. 응답 본문과 요청자를 함께 넘깁니다.

     ![촬영: 입력 지정](../assets/lab4/lab4-29.png)

30. 저장하고 다시 게시한 뒤, 24번의 문장을 다시 넣습니다.

31. 실행 기록을 열어 에이전트의 답이 돌아왔는지 봅니다.

     ![촬영: 에이전트 응답](../assets/lab4/lab4-31.png)

32. 돌아온 내용을 읽습니다. Lab 1 에서 미리 보기로 받던 것과 같습니다. **사람이 창을 열지 않았는데도 같은 결과가 나옵니다.**

### E. M365 Copilot 노드로 규정 문의를 받습니다

{: start="33" }
33. `규정 문의` 갈래에 **M365 Copilot** 노드를 추가합니다.

     ![촬영: M365 Copilot 노드](../assets/lab4/lab4-33.png)

34. 연결을 만들라고 하면 만듭니다.

35. 물어볼 내용으로 응답 본문을 넘깁니다.

     ![촬영: 질문 지정](../assets/lab4/lab4-35.png)

36. 저장하고 다시 게시한 뒤, 22번의 문장을 다시 넣습니다.

37. 답이 돌아오는지 봅니다.

     ![촬영: M365 Copilot 응답](../assets/lab4/lab4-37.png)
...
{: start="38" }
38. `대사 요청` 갈래의 에이전트 노드 뒤에 **Teams 메시지 보내기**를 붙입니다.

     ![촬영: Teams 노드](../assets/lab4/lab4-38.png)

39. 받는 사람을 요청자로 지정하고 본문에 에이전트의 답을 넣습니다.

     ![촬영: 메시지 구성](../assets/lab4/lab4-39.png)

40. `규정 문의` 갈래에도 같은 것을 붙입니다.

41. `사람 판단` 갈래에는 **담당자에게** 보냅니다. 여기는 자동으로 처리하지 않습니다.

     ![촬영: 사람 판단 갈래](../assets/lab4/lab4-41.png)

42. 양식에 24번의 문장을 넣고 Teams에 실제로 도착하는지 봅니다.

     ![촬영: Teams 도착](../assets/lab4/lab4-42.png)
...
{: start="43" }
43. `해당 없음` 갈래에 **Teams 메시지 보내기**를 추가합니다.

     ![촬영: 해당 없음 갈래의 Teams 노드](../assets/lab4/lab4-43.png)

44. 본문에 아래를 그대로 입력합니다.
...
45. 요청 본문을 함께 실어 보냅니다. 사람이 읽고 판단할 수 있어야 합니다.

46. 양식에 **비용과 관계없는 글**을 하나 넣습니다.

    ```
    사무실 형광등이 하나 나갔는데 어디에 말하면 되나요?
    ```

47. Teams에 메시지가 오는지 확인합니다.

     ![촬영: 관계없는 요청이 Teams로 온 화면](../assets/lab4/lab4-47.png)
